...
def rbf(x_1, x_2, sigma=0.5):
    ...
    '''
    diff = x_1.unsqueeze(1) - x_2.unsqueeze(0)
    sq_distances = (diff ** 2).sum(dim=2)
    kernel_values = torch.exp(-sq_distances / (2 * sigma ** 2))
    return kernel_values  ### YOUR CODE HERE
...
def hinge_loss(scores, labels):
    ...
    losses = torch.max(torch.zeros(scores.shape, requires_grad=True),
                       torch.ones(scores.shape, requires_grad=True) - labels * scores)
    return losses.mean()  ### YOUR CODE HERE
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def predict_scores(self, batch):
        with torch.no_grad():
            ...
            return (K @ self.betas).flatten()  ### YOUR CODE HERE
